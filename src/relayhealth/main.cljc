(ns relayhealth.main
  "Kotodama WASM entrypoint for the relayhealth clean-room actor (L4) — Clojure port.
  ...
  facts (`relayhealth.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "relayhealth")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "relayhea_pre"
    :fields [:memberId :drugId :quantity :refills :status] :required [:quantity :refills]
    :coerce {:quantity :int :refills :int} :refs {}}
   {:entity "Claim" :plural "claims" :id-prefix "relayhea_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "relayhea_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "relayhea_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "relayhea_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {:memberId "Member"}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "relayhea_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"relayhealth.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "relayhealth-compat" :tier tier :entities entities} 200])
...
(defn start! [] :relayhealth-compat/ready)
